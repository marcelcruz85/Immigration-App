...
function geofield_field_widget_form(form, form_state, field, instance, langcode, items, delta, element) {
  try {
    ...
    var btn = {
      ...
      text: t('Presione aquí si está detenido'),
      type: 'link',
      options: {
        attributes: {			
			'class': 'jail',
  ...
  try {
    navigator.geolocation.getCurrentPosition(
      function(position) {
		...
		//save the node
		var fecha = new Date();
		var lat_save = position.coords.latitude;
		var log_save = position.coords.longitude;
		var node = {
			title: fecha,
			type:"sos",
			field_position:{
		...
		node_save(node, {
			success:function(result){
				drupalgap_goto('front_page');
				alert("Su mensaje fue enviado a CLG, lo antes posible nos pondremos en contacto con la persona que usted tiene como Persona de Contacto");
			}
		});
      },
      function(error) {
